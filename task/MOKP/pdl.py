"""
Preference Distribution Learning (PDL) for the Multi-Objective Knapsack Problem (MOKP).
"""
...
tester_params = {
    ...
    'model_load': {
        'path': '.',   # directory of the pre-trained PMOCO model
        'epoch': 1,    # epoch checkpoint of the pre-trained model to load
    },
    'test_episodes': 100,
    'test_batch_size': 100,
...
# Path to the pre-generated MOKP test set (provided by PMOCO).
# Replace with your local path; the default keeps the file alongside this script.
TEST_DATA_PATH = './mokp200_test_seed1234.pkl'
...
def main():
    ...
    # Training hyperparameters (Section V-A.1 of the paper).
    epoch = 10000
    generator_batch_size = 64
    prob_size = 20         # number of MOKP instances used to collect training data
    instance_size = 100    # number of preferences sampled per instance (i.e., |D| per instance)

    # Initialize the Proxy and Generator models.
    Model_projection = torch.load('Model_projection.pth')   # Proxy F(lambda|omega)
    generator = MLP(2)                                       # Generator G(lambda|phi)

    # Optimizers for Proxy and Generator.
    ...
    for i in range(prob_size):
        shared_problem = get_random_problems(tester_params['test_episodes'],
        ...
        for j in range(instance_size // 2):
            ...
            pref[1] = 1 - pref[0]
            pref_list[i][j] = pref
            sols_list[i][j] = torch.tensor(tester.run(shared_problem, pref))
        ...
        for j in range(nsols):
            ...
            sols_list[i][j + a] = torch.tensor(tester.run(shared_problem, pref))
...
def eval_generator():
    with open(TEST_DATA_PATH, 'rb') as f:
        data = pickle.load(f)
    shared_problem = torch.tensor(data).cuda()
    ...
    for j in range(10):
        ...
        for i in range(n_sols):
            ...
            sols[i] = np.array(tester.run(shared_problem, pref))
        ...
        for i in range(n_sols):
            ...
            sols_2[i] = np.array(tester.run(shared_problem, pref))

        # MOKP is maximization, so we negate for the HV computation
        # (hvwfg expects minimization). Reference point follows Table I of the paper.
        ref = np.array([-40, -40])
    ...
    # Optional correction factor for endpoint preferences (see Section IV-B,
    # "noisy points at the end of Pareto front").
    left = generator(torch.tensor([[1., 0.]]))[0]
    right = generator(torch.tensor([[0., 1.]]))[0]
    k = torch.abs(left[0] - right[0])
    correction_ratio = k * max_ratio / (k * max_ratio + 1 - max_ratio)

    return max_ratio, correction_ratio, hv_list, k
